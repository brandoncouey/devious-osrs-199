...
public class TargetRoute {
    ...
    private void beforeMovement0(Entity entity) {
        if (target.isNpc()) {
            ...
            if (name.equalsIgnoreCase("Zulrah")) {
                //System.out.println("max distance = " + distance + " > actual distance = " + entity.getPosition().distance(target.getPosition()) + " in range? " + inRange(entity, target, distance));

                if (inRange(entity, target, distance)) {
                    withinDistance = true;
    ...
    protected static boolean inTarget(int absX, int absY, int size, int targetX, int targetY, int targetSize) {
        if (absX > (targetX + (targetSize - 1)) || absY > (targetY + (targetSize - 1)))
            return false;
        return targetX <= (absX + (size - 1)) && targetY <= (absY + (size - 1));
    }

    public static boolean inRange(Entity entity, Entity target, int distance) {
        return inRange(entity.getAbsX(), entity.getAbsY(), entity.getSize(), target.getAbsX(), target.getAbsY(), target.getSize(), distance);
    }
