...
public class DumbRoute {
    ...
    private static boolean allowEntrance(ClipUtils clipUtils, int x, int y, int z, int size, Direction dir) {
        ...
        if(size == 1) {
            ...
            if(dx == -1 && dy == 1 && (clipUtils.getAbs(x - 1, y + 1, z) & NORTH_WEST_MASK) == 0 && (clipUtils.getAbs(x - 1, y, z) & WEST_MASK) == 0 && (clipUtils.getAbs(x, y + 1, z) & NORTH_MASK) == 0)
                return true;
            return dx == 1 && dy == 1 && (clipUtils.getAbs(x + 1, y + 1, z) & NORTH_EAST_MASK) == 0 && (clipUtils.getAbs(x + 1, y, z) & EAST_MASK) == 0 && (clipUtils.getAbs(x, y + 1, z) & NORTH_MASK) == 0;
        } else if(size == 2) {
            ...
            if(dx == -1 && dy == 1 && (clipUtils.getAbs(x - 1, y + 1, z) & 0x124013e) == 0 && (clipUtils.getAbs(x - 1, y + 2, z) & NORTH_WEST_MASK) == 0 && (clipUtils.getAbs(x, y + 2, z) & 0x12401f8) == 0)
                return true;
            return dx == 1 && dy == 1 && (clipUtils.getAbs(x + 1, y + 2, z) & 0x12401f8) == 0 && (clipUtils.getAbs(x + 2, y + 2, z) & NORTH_EAST_MASK) == 0 && (clipUtils.getAbs(x + 2, y + 1, z) & 0x12401e3) == 0;
        } else {
    ...
    public static void step(Entity entity, Position pos) {
        step(entity, pos.getX(), pos.getY());
    }
}
