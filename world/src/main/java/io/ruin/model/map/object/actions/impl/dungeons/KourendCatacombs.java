...
public class KourendCatacombs {
    ...
    static { // Object actions
        ...
        ItemObjectAction.register(28900, (player, item, obj) -> {
            Item shards = player.getInventory().findItem(19677);
            if (item.getId() != 19677) {
                player.sendMessage("Nothing interesting happens.");
                return;
            }
            if (shards.getAmount() < 30) {
                player.sendMessage("You do not have enough ancient shards to exchange");
                return;
            }
            item.remove(30);
            player.getInventory().addOrDrop(19685, 1);
            player.sendMessage("You Exchange 60 " + item.getDef().name + " for a Dark Totem");
        });
        ...
        ObjectAction.register(28893, "jump-to", (player, obj) -> { // Stepping stones
            ...
            try {
                ...
                Server.logError("Error invoking forced movement: player=" + player.getPosition().toString() + ", obj=" + obj);
            }
        });
