...
public enum Potion {
    ...
    Potion(int lvlReq, double xp, String potionName, String primaryName, int amounttoUse, String... secondaryNames) {
        this.lvlReq = lvlReq;
        this.xp = xp;
        this.potionName = potionName;
        this.primaryName = primaryName;
    ...
    private void mix(Player player, Item primaryItem, List<Item> secondaryItems) {
        ...
        if (potionName.contains("weapon poison")) {
            int amount = player.getInventory().count(15267);
            player.getInventory().add(187, amount);
            player.getInventory().remove(15267, amount);
        }
