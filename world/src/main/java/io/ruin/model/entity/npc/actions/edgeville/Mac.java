...
public class Mac {
    ...
    static {
        ...
        ShopItem compCape = ShopItem.builder()
                .id(30125)
                .amount(100)
                .price(ECO_PRICE * 23)
                .additionalItems(Arrays.asList(new Item(30123, 1)))
                .requirementCheckType(RequirementCheckType.REQUIRED_TO_BUY)
                .additionalRequirements(player -> MaxCape.doneAchieves(player) ? "" : "You need to be maxed in all trainable stats and finished all achievements .")
                .build();
        trimmedCapes.add(compCape);
