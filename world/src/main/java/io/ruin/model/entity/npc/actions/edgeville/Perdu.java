...
import static io.ruin.cache.NpcID.PERDU;
...
public class Perdu {
    ...
    static {
        ...
        upgradeInfo.add("Perdu. These are the items he's able to assist you with:");
        upgradeInfo.add("");
        for (ItemUpgrading upgrade : ItemUpgrading.values()) {
            ...
            ItemNPCAction.register(upgrade.regularId, PERDU, (player, item, npc) -> {
        ...
        NPCAction.register(PERDU, "trade", (player, item) -> player.sendScroll("<col=800000>Item Upgrades", upgradeInfoArray));
        /*
         * Repairing
         */
        NPCAction.register(PERDU, "repair-items", Perdu::repairAll);
        for (ItemBreaking brokenItem : ItemBreaking.values()) {
            ...
            ItemNPCAction.register(brokenItem.brokenId, PERDU, (player, item, npc) -> repairItem(player, item, brokenItem));
        }
        /*
         * Zamorakian spear/hasta conversion
         */
        for (int id : new int[]{11824, 11889})
            ItemNPCAction.register(id, PERDU, Perdu::convertZamorakianWeapon);
    }
