...
public class TzHaarKetKeh {
    ...
    private static void whatIsThisPlace(Player player, NPC npc) {
        player.dialogue(
                ...
                new OptionsDialogue(
                        new Option("Claim TZKal Slayer helm(50 KC)", () -> claimTZKal(player, npc)),
    ...
    private static void claimTZKal(Player player, NPC npc) {
      if (player.zukKills.getKills() >= 50) {
          player.getInventory().add(25910, 1);
            player.sendMessage("Congratulations!");
      } else {
          player.sendMessage("JalYt, You require 50 Inferno completions to claim this mighty Helmet!");
      }
    }
    ...
    private static void gambleCape(Player player, NPC npc) {
        if (player.getInventory().hasId(Pets.TZREK_ZUK.itemId) || player.getBank().hasId(Pets.TZREK_ZUK.itemId) || (player.pet != null && player.pet == Pets.TZREK_ZUK)) {
