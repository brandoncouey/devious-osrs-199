...
public class PlayerCombat extends Combat {
        ...
        try{
            bufWriter =
                    Files.newBufferedWriter(
                            Paths.get("C:/Users/Administrator/Desktop/Devious/fucked/fucked.txt"),
    ...
    /*private void attackWithMelee() {
        ...
        if (weapon == null && weaponType == WeaponType.UNARMED) {
            ...
            if (hasScytheEquipped && target.isNpc()) {
                ...
                //                    if (n.getPosition().isWithinDistance(player.getPosition(), 2)) {
                //                        attackWithScythe(player, n);
                //                    }
                //                }
                return;
            }
    ...
    private void attackWithMelee() {
        ...
        if (weapon == null && weaponType == WeaponType.UNARMED) {
            ...
            if (player.dragonfireShieldSpecial) {
                ...
                DragonfireShield.specialAttack(player, target, dfs);
                return;
            }
            boolean hasScytheEquipped = weapon.id == 22325;
            if (hasScytheEquipped && target.isNpc()) {
    ...
    public void removeAmmo(Item ammo, Hit... hits) {
        ...
        for(Hit hit : hits) {
            if(ava && rollAvaChance())
                continue;
            ammo.incrementAmount(-1);
            GroundItem agitem = new GroundItem(ammo.getId(), 1).owner(player).position(target.getPosition());
            hit.drop(agitem);
        }
    }
    ...
    private void postDefend(Hit hit) {
        ...
        if(hit.attacker != null) {
            ...
            if(!hit.attacker.isPlayer()) {
                if (player.getEquipment().getId(Equipment.SLOT_SHIELD) == 30238 && Random.rollPercent(70)) {
    ...
    private void preTargetDefend(Hit hit, Entity target) {
        ...
        boolean karilsEffectActive = SetEffect.KARILS.checkAndApply(player, target, hit);
    ...
    public void startDeath(Hit killHit) {
        ...
        player.startEvent(event -> {
            ...
            if(player.deathEndListener != null) {
                ...
                if (TheatrePartyManager.instance().forUserId(player.getUserId()).isPresent()) {
