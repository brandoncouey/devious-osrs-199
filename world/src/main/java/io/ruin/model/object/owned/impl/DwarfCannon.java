...
/*        if (getOwner().storeAmountSpent > 5) {
    ...
    public boolean handleAreaRestriction() {
        ...
        if (getOwner().getPosition().regionId() == 10036) {
            getOwner().sendMessage("You can't deploy a cannon here.");
            return false;
        }
        return true;
...
//        }
        ...
        LoginListener.register(player -> {
            if (player.cannonLost)
                player.sendMessage(Color.RED.wrap("Your cannon has been destroyed, you can reclaim it from the Drunken Dwarf at home."));
        });
    }
