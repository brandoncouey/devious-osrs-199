    ...
    private static final class WorldSwitcher {
        ...
        static {
            InterfaceHandler.register(Interface.WORLD_SWITCHER, h -> {
                h.actions[3] = (SimpleAction) p -> p.closeInterface(InterfaceType.INVENTORY);
                h.actions[23] = (SimpleAction) Player::attemptLogout;
                ...
                h.actions[16] = (DefaultAction) (player, childId, option, slot, itemId) -> {
                    if (option == 1)
                        ...
                        player.sendMessage("Failed to hop world. Try again later");
                };
            });
        }

        public static void open(Player player) {
                player.openInterface(InterfaceType.INVENTORY, Interface.WORLD_SWITCHER);
