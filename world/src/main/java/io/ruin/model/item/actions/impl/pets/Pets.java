...
public enum Pets {
    ...
    TZREK_JAD(13225, 5893, 10625, true, 100),
    JALREK_JAD(25519, 10625, 5893, true, 100),
...
//                System.out.println(itemDef);
     ...
     * Skilling pets
     * */
    BABY_CHINCHOMPA_GREY(13324, 6757, 6756, true, 200, new BonusExperiencePerk() {
        @Override
    ...
    BABY_CHINCHOMPA_RED(13323, 6756, 6758, false, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Hunter) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    BABY_CHINCHOMPA_BLACK(13325, 6758, 6757, false, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Hunter) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    BABY_CHINCHOMPA_GOLD(13326, 6759, 6757, false, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Hunter) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    BEAVER(13322, 6724, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
    ...
    HERON(13320, 6722, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
    ...
    ROCK_GOLEM(13321, 7451, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
    ...
    GIANT_SQUIRREL(20659, 7351, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
    ...
    TANGLEROOT(20661, 7352, false, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
    ...
    ROCKY(20663, 7353, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
    ...
    RIFT_GUARDIAN_FIRE(20665, 7354, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
    ...
    RIFT_GUARDIAN_AIR(20667, 7355, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_MIND(20669, 7356, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_WATER(20671, 7357, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_EARTH(20673, 7358, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_BODY(20675, 7359, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_COSMIC(20677, 7360, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_CHAOS(20679, 7361, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_NATURE(20681, 7362, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_LAW(20683, 7363, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_DEATH(20685, 7364, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_SOUL(20687, 7365, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_ASTRAL(20689, 7366, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    RIFT_GUARDIAN_BLOOD(20691, 7367, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
            if (statType == StatType.Runecrafting) { //checks if it is skill
                return 10; //if yes return bonus if not go to the next statement
    ...
    HERBI(21509, 7760, true, 200, new BonusExperiencePerk() {
        @Override
        public double getBonusExpPercentage(Player player, StatType statType) {
     ...
     * Golems
     */
    TIN_GOLEM(21187, 7440, true, 200),
    CLAY_GOLEM(21188, 7441, true, 200),
    IRON_GOLEM(21189, 7442, true, 200),
    BLURITE_GOLEM(21190, 7443, true, 200),
    SILVER_GOLEM(21191, 7444, true, 200),
    COAL_GOLEM(21192, 7445, true, 200),
    GOLD_GOLEM(21193, 7446, true, 200),
    MITHRIL_GOLEM(21194, 7447, true, 200),
    ADAMANT_GOLEM(21196, 7449, true, 200),
    RUNITE_GOLEM(21197, 7450, true, 200),

    /**
     * Phoenixs
     */
    GREEN_PHOENIX(24483, 3077, true, 200),
    BLUE_PHOENIX(24484, 3078, true, 200),
    PURPLE_PHOENIX(24486, 3080, true, 200),
    WHITE_PHOENIX(24485, 3079, true, 200),
    ...
    WALKER(26479, 14745, false),
    REVENANT(30348, 15105, false),
    NEXLING(30349, 15107, false),
    ...
    Pets(int itemId, int npcId, boolean mysteryBox, Perk... perks) {
        this(itemId, npcId, -1, mysteryBox, 0, perks);
    }

    Pets(int itemId, int npcId, int metaId, boolean mysteryBox, Perk... perks) {
        this(itemId, npcId, metaId, mysteryBox, 0, perks);
    }

    Pets(int itemId, int npcId, boolean mysteryBox, int dropAverage, Perk... perks) {
        this(itemId, npcId, -1, mysteryBox, dropAverage, perks);
    }

    Pets(int itemId, int npcId, int metaId, boolean mysteryBox, int dropAverage, Perk... perks) {
        this.itemId = itemId;
        this.npcId = npcId;
        this.metaId = metaId;
    ...
    static {
            ...
            NPCAction.register(pet.npcId, "metamorphosis", (player, npc) -> {
                ...
                if (pet == TZREK_JAD) {
                    if (!player.infernalJadMetamorphisis) {
                        player.sendMessage("You need to use 150k Blood Money on this first before you can use metamorphosis.");
                        return;
                    }
                }
        ...
        ItemNPCAction.register(13307, TZREK_JAD.npcId, (player, item, npc) -> {
            if (!player.infernalJadMetamorphisis) {
                player.sendMessage("You've unlocked the JalTok-Jad metamorphosis for your Tzrek Jad.");
                item.remove(150000);
                player.infernalJadMetamorphisis = true;
            } else {
    ...
    public static boolean pickup(Player player, NPC npc, Pets pet) {
    ...
    private static Consumer<Player> getTalkAction(Pets pet) {
        switch (pet) {
            case ABYSSAL_ORPHAN: {
    ...
    public static Pets forNpcId(int spawnNpcId) {
        for (var p : Pets.values()) {
    ...
    public static int getCurrentPetLevel(final Player player) {
        if (player.pet == null)
            return -1;
        final Pets pet = forNpcId(player.pet.npcId);
        if (!player.getPetStatistics().containsKey(pet)) {
            player.getPetStatistics().put(pet, new PetData());
        }
        return player.getPetStatistics().get(pet).getLevel();
    }

    public static void addExperienceForPet(final Player player, final Pets pet, int experience) {
        PetData data = player.getPetStatistics().containsKey(pet) ? player.getPetStatistics().get(pet) : new PetData();
        final int level = data.getLevel();
        data.setExperience(data.getExperience() + experience);
        if (!player.getPetStatistics().containsKey(pet)) {
            player.getPetStatistics().put(pet, data);
        }
        final int newLevel = data.getLevel();
        if (newLevel > level) {
            player.sendMessage("Your " + NPCDef.get(pet.npcId).name + " has just reached level " + newLevel + "!");
        }
    }

    public static void init() {
        for (Position position : POSITIONS) {
            final int npcId = Pets.values()[Misc.random(0, Pets.values().length - 1)].npcId;
            new NPC(npcId).spawn(position.getX(), position.getY(), position.getZ(), Direction.get("W"), 1);
        }
    }

    private static final Position[] POSITIONS = new Position[]{
            new Position(3110, 3473, 0),
            new Position(3116, 3475, 0),
            new Position(3117, 3478, 0),
            new Position(3122, 3473, 0),
            new Position(3124, 3476, 0),
            new Position(3121, 3483, 0),
            new Position(3124, 3483, 0),
            new Position(3126, 3480, 0),
            new Position(3114, 3468, 0),
    };
