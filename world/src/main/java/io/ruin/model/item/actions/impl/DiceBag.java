    ...
    public static void roll(Player player, int sides, boolean rollHigh, boolean rollLow) {
        ...
        player.stringInput("Enter player's display name:", name -> {
            ...
            player.startEvent(event -> {
                ...
                player.lock();
                player.forceText("<img=23> Rolling against " + target.getName() + "...");
                player.sendFilteredMessage("You roll the dice..");
                event.delay(3);
                player.forceText("<img=23> ...Rolled a " + roll + "/" + sides + "");
                player.sendMessage("<img=23> You rolled a " + roll + "/" + sides + " on the dice.");
                target.sendMessage("<img=23> " + player.getName() + " rolled a " + roll + "/" + sides + " on the dice.");
                player.unlock();
            });
        });
