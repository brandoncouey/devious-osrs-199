...
public class LootingBag extends ItemContainer {
    ...
    public static final int CLOSED_LOOTING_BAG2 = 30453;

    public static final int OPENED_LOOTING_BAG2 = 30454;
    ...
    private void deposit(Item item, int amount) {
        ...
        if (player.wildernessLevel == 0 && !World.isPVPWorld()) {
    ...
    private static final int[] LOOTING_BAGS2 = {
            OPENED_LOOTING_BAG2, CLOSED_LOOTING_BAG2
    };
    ...
    public static void destroy2(Player player, Item item) {
        player.dialogue(
                new YesNoDialogue("Are you sure you want to do this?", "This Looting bag will be destroyed and all stored items will be dropped.", item, () -> {
                    //item.remove();
                    player.sendMessage("You empty the contents of your looting bag, they will appear publicly in 2 hours.");
                    for (Item i : player.getLootingBag().getItems()) {
                        if (i != null) {
                            new GroundItem(i).owner(player).droppedBy(player).position(player.getPosition()).spawnPrivate();
                            i.remove();
    ...
    static {
        InterfaceHandler.register(Interface.LOOTING_BAG, h -> {
            ...
            for (int LOOTING_BAG : LOOTING_BAGS) {
                ...
                for (int LOOTING_BAG2 : LOOTING_BAGS2) {
                    ItemItemAction.register(LOOTING_BAG2, (player, lootingBag, itemUsed) -> {
                        if (!player.getInventory().hasMultiple(itemUsed.getId())) {
                            player.getLootingBag().deposit(itemUsed, 1);
                            return;
                        }
                ...
                ItemAction.registerInventory(LOOTING_BAG2, "check", (player, item) -> player.getLootingBag().openCheck());
                ItemAction.registerInventory(LOOTING_BAG2, "deposit", (player, item) -> player.getLootingBag().openDeposit());
                ItemAction.registerInventory(LOOTING_BAG2, "destroy", LootingBag::destroy2);
            ...
            ItemAction.registerInventory(CLOSED_LOOTING_BAG2, "open", (player, item) -> {
                item.setId(OPENED_LOOTING_BAG2);
                player.sendFilteredMessage("You open your looting bag, ready to fill it.");
            });

            ItemAction.registerInventory(OPENED_LOOTING_BAG2, "close", (player, item) -> {
                item.setId(CLOSED_LOOTING_BAG2);
                player.sendFilteredMessage("You close your looting bag.");
            });
        }
