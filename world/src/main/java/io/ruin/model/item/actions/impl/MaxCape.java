...
public class MaxCape {
    ...
    public static boolean doneAchieves(Player player) {
        return player.getDiaryManager().getKourendDiary().hasDoneAll() && player.getDiaryManager().getArdougneDiary().hasDoneAll()
                && player.getDiaryManager().getWesternDiary().hasDoneAll() && player.getDiaryManager().getFaladorDiary().hasDoneAll()
                && player.getDiaryManager().getFremennikDiary().hasDoneAll() && player.getDiaryManager().getKandarinDiary().hasDoneAll()
               && player.getDiaryManager().getKaramjaDiary().hasDoneAll() && player.getDiaryManager().getLumbridgeDraynorDiary().hasDoneAll()
    ...
    static {
        ...
        ItemDef.forEach(def -> def.maxType = def.name.toLowerCase().contains("max cape") || def.name.toLowerCase().contains("max hood") || def.name.toLowerCase().contains("imbued zamorak max")  || def.name.toLowerCase().contains("imbued guthix max") || def.name.toLowerCase().contains("imbued saradomin max"));
    }
