...
public enum DragonfireShield {
    ...
    static {
        for(DragonfireShield shield : DragonfireShield.values()) {
             ...
             * Activate
             */
            ItemAction.registerEquipment(shield.chargedId, "activate", (player, item) -> {
                if (AttributeExtensions.getCharges(item) > 0) {
                    player.dragonfireShieldSpecial = true;
                    player.sendFilteredMessage("You activate your dragonfire shield");
                } else {
                    player.sendFilteredMessage("Nah mate something else is happening ....");
                }
            });
        }
    }
    ...
    private static void charge(Player player, Item shield) {
        ...
        if(charges >= MAX_CHARGES)
            return;
        shield.putAttribute(AttributeTypes.CHARGES, charges + 1);
    ...
    public static void specialAttack(Player player, Entity target , Item item) {
        ...
        if (!wearingDragonfireShield(player)) {
            player.dragonfireShieldSpecial = false;
            player.sendMessage("you don't have a dragon fire shield");
            return;
        }
        if(player.isLocked()) {
            player.dragonfireShieldSpecial = false;
            player.sendMessage("you're locked and cannot do this.");
            return;
        }
        ...
        if(target == null) {
            player.dragonfireShieldSpecial = false;
            player.sendMessage("that target is nulled");
            return;
        }
        int shieldId = item.getId();
        if(!consumeCharge(player, item)) {
            player.sendMessage("cannot consume charges.");
            return;
        }
