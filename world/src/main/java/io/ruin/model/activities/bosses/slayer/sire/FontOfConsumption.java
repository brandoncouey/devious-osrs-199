...
public class FontOfConsumption {
    ...
    static {
        ItemObjectAction.register(25624, 27029, (player, item, obj) -> {
            player.startEvent(event -> {
                player.lock();
                player.face(Direction.NORTH);
                event.delay(1);
                player.dialogue(new ItemDialogue().one(25624, "You place the Unsired into the Font of Consumption...").hideContinue());
                player.animate(827);
