...
public class ChambersOfXeric {
    ...
    static {
        ...
        ObjectAction.register(29734, 1, (player, obj) -> { // hole to lower level
            if (player.raidsParty == null || player.raidsParty.getRaid() == null)
                return;
            getRaidForPlayer(player).enterLowerFloor(player);
        });

        ObjectAction.register(29995, 1, (player, obj) -> {
            if (player.raidsParty == null || player.raidsParty.getRaid() == null)
                return;
            getRaidForPlayer(player).leaveLowerFloor(player);
        });
        ...
        ObjectAction.register(29735, 1, (player, obj) -> {
            if (player.raidsParty == null || player.raidsParty.getRaid() == null)
                ...
                return;
            getRaidForPlayer(player).enterOlmFloor(player);
        });

        ObjectAction.register(29996, 1, (player, obj) -> {
            if (player.raidsParty == null || player.raidsParty.getRaid() == null)
                return;
            getRaidForPlayer(player).leaveOlmFloor(player);
        });
        ...
        ObjectAction.register(29778, 1, (player, obj) -> {
            if (isRaiding(player) && getRaidForPlayer(player).isComplete()) {
                leaveRaid(player);
            } else {
                player.dialogue(new OptionsDialogue(isRaiding(player) && getRaidForPlayer(player).isStarted() ?
    ...
    private final RaidStorage storage = new RaidStorage(this);

    private final List<GameObject> storageObjects = new ArrayList<>();
    ...
    public static void createChamber(Player player, Party party) {
        ...
        if (party.getRaid() != null) {
            getRaidForPlayer(player).enterRaid(player);
            return;
        }
        ...
        player.startEvent(event -> {
            ...
            player.getPacketSender().fadeOut();
            event.delay(2);
            player.getMovement().teleport(getRaidForPlayer(player).getStartingChamber().getEntrancePosition());
            raid.entered(player);
    ...
    private void exited(Player player, boolean logout) {
        ...
        JournalTab.swap(player, Interface.QUEST_TAB);
    }
    ...
    public static void addDamagePoints(Player player, NPC target, int points) {
        ...
        if (target.get("RAID_NO_POINTS") != null)
            return;
//        points /= 2;
        points *= getRaidForPlayer(player).combatPointsFactor;
    ...
    private static ChambersOfXeric getRaidForPlayer(Player player) {
        return player.raidsParty.getRaid();
    }

    private static ChambersOfXeric getRaidForPart(Party party) {
        return party.getRaid();
    }
