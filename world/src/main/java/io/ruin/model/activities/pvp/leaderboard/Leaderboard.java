...
public class Leaderboard {
     ...
     */
    private static final List<Reward> unclaimedRewards = new ArrayList<>();
    ...
    static {
        ...
        ObjectAction.register(3192, "View", ((player, obj) -> openEdgeLeaderBoard(player)));
    }
    ...
    private static void openEdgeLeaderBoard(Player player) {
        ...
        player.getPacketSender().sendString(108, 20, "Devious Pking Leaderboard");
    ...
    public static void openKillFeed(Player player) {
        player.openInterface(InterfaceType.MAIN, 90);
        for (int i = 1; i < 55; i++) {
            player.getPacketSender().sendString(90, i, "");
        }
        player.getPacketSender().sendString(90, 55, "");
        player.getPacketSender().sendString(90, 56, "");
        if (edgePkers != null && !edgePkers.isEmpty()) {
            Set<Entry<Integer, EdgePker>> map = sortByValuesEdge(edgePkers);
            Iterator iterator2 = map.iterator();
            int index = 1;
            while (iterator2.hasNext()) {
                Map.Entry entry = (Map.Entry) iterator2.next();
                EdgePker pker = ((EdgePker) entry.getValue());
                player.getPacketSender().sendString(90, 55 + index, index + ": " + pker);
                index++;
                if (index + 55 == 56) {
                    break;
                }
    ...
    public static void pullWinners() {
        World.startEvent(event -> {
            ...
            if (topThreeEdge != null) {
                ...
                Item firstPlaceReward = new Item(290, 2);
                Item secondPlaceReward = new Item(13307, 75000);
                Item thirdPlaceReward = new Item(13307, 45000);
