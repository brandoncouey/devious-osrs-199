...
public enum BarrowsBrother {
    ...
    public void spawn(Player player) {
        ...
        npc.deathStartListener = (entity, killer, killHit) -> {
            if (player.barrowsKills.getKills() <= 1) {
                if (killHit.attackStyle.isMagic()) {
                    player.openInterface(InterfaceType.SECONDARY_OVERLAY, 660);
                    player.getPacketSender().sendClientScript(3343, "iss", 0xff981f, "Combat Task Completed!", "Task Completed:" + Color.WHITE.wrap(" Defence? What Defence?"));
                    Config.DEFENCE_WHAT_DEFENCE.set(player, 1);
