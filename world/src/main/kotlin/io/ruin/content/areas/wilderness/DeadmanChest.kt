...
class DeadmanChest(val location: DeadmanChestLocation) {
    ...
    fun loot(player: Player) {
        ...
        val removedKey = player.inventory.remove(CHEST_KEY_ID, 20_000_000)
        if (removedKey == 0) {
            player.sendMessage("The chest requires 20m GP to open")
            return
        }
        val rdt = DROP_TABLE.rollItem()
        player.inventory.addOrDrop(13307, Random.get(150, 600))
        player.inventory.addOrDrop(rdt)
        player.hit(Hit().randDamage(15))
    ...
    companion object {
        const val CHEST_KEY_ID = 995

        val DROP_TABLE = LootTable()
            .addTable(
                5,
                LootItem(21295, 1, 1),
                LootItem(22981, 1, 1),
