...
class LastManStandingSession(private val queue: LastManStandingQueue) {
     ...
     */
    private val BOOSTED_STATS = arrayOf(
        StatType.Attack,
        StatType.Defence,
        StatType.Strength,
        StatType.Hitpoints,
     ...
     */
    @JvmField
    var crate: LastManStandingCrate? = null
    ...
    private fun Player.remove() {
        ...
        normalize()
        equipment.clear()
        player.sendMessage("You have competed in: $lmsGamesPlayed LMS Games")
        inventory.clear()
        giveReward()
        participants -= this
    ...
    fun exchangeKey(player: Player) {
        val key = player.inventory.items.filter(Objects::nonNull)
            .find { it.id == LastManStanding.BLOODY_KEY || it.id == LastManStanding.BLOODIER_KEY }
        if (key != null) {
            player.lock()
            player.rollLoot(key)
            player.heal()
    ...
    private fun Player.setAttributes() {
        ...
        preTournyAttack =
            intArrayOf(stats.get(StatType.Attack).fixedLevel, stats.get(StatType.Attack).experience.toInt())
        preTournyStrength =
            intArrayOf(stats.get(StatType.Strength).fixedLevel, stats.get(StatType.Strength).experience.toInt())
        preTournyDefence =
            intArrayOf(stats.get(StatType.Defence).fixedLevel, stats.get(StatType.Defence).experience.toInt())
        preTournyRanged =
            intArrayOf(stats.get(StatType.Ranged).fixedLevel, stats.get(StatType.Ranged).experience.toInt())
        preTournyPrayer =
            intArrayOf(stats.get(StatType.Prayer).fixedLevel, stats.get(StatType.Prayer).experience.toInt())
        preTournyMagic = intArrayOf(stats.get(StatType.Magic).fixedLevel, stats.get(StatType.Magic).experience.toInt())
        preTournyHitpoints =
            intArrayOf(stats.get(StatType.Hitpoints).fixedLevel, stats.get(StatType.Hitpoints).experience.toInt())
    ...
    private fun broadcastSafeSpot() {
        participants.forEach {
            it.packetSender.sendMessage(
                "Lethal fog will soon fill the island! The safezone is the ${FOG_SEQ.title}.",
                "",
                14
            )
        }
