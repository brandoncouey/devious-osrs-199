...
class Tournament(var attributes: TournamentAttributes) {
     ...
     */
    var stage = TournamentStage.NONE
...
/*            player.teleportListener = TeleportListener { p: Player ->
    ...
    private fun startRound(lastRound: Boolean) {
        ...
        for (index in 0 until copy.size) {
            ...
            val arenaTile =
                if (finalRound) TournamentMap.FINALE_ARENA_CENTRE_TILE else TournamentMap.BATTLE_ARENA_CENTRE_TILES[index]
            if (pair.first != null) {
                val plr = pair.first!!
                plr.refresh()
...
/*        if (player.tournamentPouch) {
    ...
    private fun Player.resolvePrize() {
        when (contestants) {
            3 -> {
                inventory.add(20527, 1)
                message(Color.RED.wrap("Congratulations! You have placed third within the tournament. You have won 1 PVP Tokens"))
            }
            2 -> {
                inventory.add(20527, 2)
                message(Color.RED.wrap("Congratulations! You have placed second within the tournament. You have won 2 PVP Tokens"))
            }
            1 -> {
                inventory.add(20527, 3)
                message(Color.RED.wrap("Congratulations! You have placed first within the tournament. You have won 3 PVP Tokens"))
            }
            else -> {
